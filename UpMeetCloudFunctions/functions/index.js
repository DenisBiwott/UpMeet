...
admin.initializeApp({
  databaseURL: "https://buuk-rides.firebaseio.com"
});
...
var consumer_key, consumer_secret, url, auth, oauth_token;
...
var db = admin.database();
 ...
 // ---------- MPESA  STK REQUEST ------------------
 exports.koskiRequest = functions.https.onRequest(async (requestf, _response) => {
    ...
    function (error, response, body) {
      ...
      request(
        ...
        json : {
          ...
          "CallBackURL": "https://us-central1-buuk-rides.cloudfunctions.net/koskiCallback",
...
// ----------  MPESA CALLBACK URL ------------------
exports.koskiCallback = functions.https.onRequest(async (requestf, _response) => {
...
// BOOKINGS
exports.sendBookingNotification = functions.database.ref('/Notifications/{OwnerUID}/{CustomerUID}')
    .onWrite(async (change, context) => {
      ...
      .ref(`/Users/${CustomerUID}`).once('value');

      return getUserName.then(fromUserResult =>  {

        const name = fromUserResult.val().username;
        ...
        return Promise.all([getDeviceTokensPromise]).then(result => {
          ...
          const payload = {
            notification: {
              title: 'You have a new StoreRequest!',
              body: `${name} made a request.`,
              icon: 'default'
              // body: ` made a request.`,
